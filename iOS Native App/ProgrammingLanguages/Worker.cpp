...
std::map<std::string, std::string> Worker::dictionaryData() {
    ...
    dataDict["Price per Hour"] = std::to_string(this->pricePerHour);
    dataDict["Productivity coefficent"] = std::to_string(this->productivityCoef);
    dataDict["Work Experience"] = std::to_string(this->workExperience);
